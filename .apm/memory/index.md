...
- Chamadas ao Prisma dentro de `runWithTenant` (isolamento multi-tenant) precisam de `await` interno na própria função que as executa — sem isso, o despacho real da query ocorre fora da janela síncrona do `AsyncLocalStorage.run()` e o contexto de tenant se perde (falha com `MissingTenantContextError`, não vaza dados, mas quebra a operação). Esse padrão reincidiu uma segunda vez (Task 3.5, desta vez em código de teste) — usuário aprovou uma Task dedicada (3.6) para adicionar uma salvaguarda estrutural (wrapper ou lint) em vez de depender só de documentação.
