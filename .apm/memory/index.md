...
- **Bug confirmado (verificação manual do Manager, pendente de correção na Task 2.6):** `pdf-parse`/`getTable()` corta silenciosamente (sem quebra, sem hífen, sem aviso) qualquer trecho sem espaço maior que **20 caracteres** na coluna de anilha — reprodução determinística: `"A".repeat(n)` para `n <= 20` extrai completo, `n > 20` sempre corta para exatamente 20 caracteres. É perda de dados real (diferente do bug do hífen, que preservava todos os caracteres), no identificador físico único da ave.
