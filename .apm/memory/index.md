...
- **Bug confirmado (verificação manual do Manager, pendente de correção na Task 2.5):** `pdf-parse`/`getTable()` insere um hífen literal (`-\n`) quando um valor de célula quebra em duas linhas sem espaço natural no ponto de quebra — a normalização atual (`\s+` → espaço) só trata quebras com espaço, deixando o hífen espúrio (ex: `"VERIFY-E2E-0001"` → `"VERI- FY-E2E-0001"`). Reproduzido com anilha no formato real do IBAMA (23 caracteres) e nome de espécie longo. Corrompe o identificador físico único da ave silenciosamente — a revisão manual (Task 2.4) não necessariamente chama atenção, já que o valor corrompido ainda parece plausível.
