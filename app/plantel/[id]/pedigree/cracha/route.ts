...
import CrachaDocument from "@/lib/pedigree/CrachaDocument";
...
export async function GET(_request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const buffer = await renderToBuffer(CrachaDocument({ dados }));

    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="cracha-${dados.ave.anilha}.pdf"`,
      },
    });
  } catch (error) {
